...
No — a Count-Min Sketch is the right tool when approximate is genuinely acceptable and the win is memory, not when a user will notice and object to their own count being slightly off. [Real-Time Leaderboard](../real-time-leaderboard/00-overview.md), where every user can see and compare their own exact rank, needs exact counting; this heavy-hitters log analysis, where nobody is checking one specific visitor's exact count, doesn't.
