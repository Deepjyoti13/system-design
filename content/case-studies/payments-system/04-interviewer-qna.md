...
**1. What happens when two requests hit the same resource at the same instant — specifically, the same idempotency key submitted twice concurrently?**
...
**2. What happens when traffic spikes 10x for an hour (a major sale event)?**
The Orchestration Service and processor-call tier scale horizontally since both are stateless per request; the harder constraint is the external processor's own rate limit, which this system can't scale around — request queuing with the same bounded-timeout-and-retry discipline as the normal path absorbs the burst, and nothing on the payment write path is shed, per the Load Handling section in Module 01.

**3. What if the processor actually succeeded, but the response was lost before your service saw it — doesn't a retry now double-charge?**
...
**4. How do refunds interact with the ledger — do you modify the original entries?**
...
**5. Would you ever accept eventual consistency here, the way this guide's [Counting a Billion Likes](../../../like-counting-at-scale/00-overview.md) case study does for a like count?**
...
**6. What's the failure mode if the webhook relay itself goes down?**
...
**7. Why record intent locally before calling the processor, instead of calling the processor first and recording the result?**
...
**8. How would you test this without hitting a real payment processor on every test run?**
...
**9. Would you shard `payment_intents` and `ledger_entries` by the same key as a typical high-write case study in this guide, like sharding by a UUID hash?**
...
**10. Why isn't this a distributed transaction across your database and the processor, using something like two-phase commit?**
Because the processor is a third party you don't control and can't enlist as a participant in your commit protocol — no real card network exposes a 2PC "prepare" phase. The [transactional outbox](../../hld-building-blocks/transactional-outbox-cdc.md) sidesteps the problem instead of solving it head-on: only ever commit atomically against your *own* database, and treat the external call as a step that can fail or time out independently, with a state machine and a reconciliation job designed around exactly that possibility.
